...
#    logging.debug("current directory is: %s", ftp_connection.pwd())
    dirname = "/logfiles/6824Sherwick/"
    logging.info("Variable ftp_connection: %s", ftp_connection)
    logging.info("Variable dirname: %s", dirname)
...
def storefile(ftp_dir, filepath, donepath, filename, today):
    ...
    if ftp_connection != None:
        ...
        try:
            ...
            filehandle.close()
            logging.info("Moving file : %s to ftp", filename)
...
def save_log_file_today():      #To save today's file name
    todaydate = datetime.date.today()
    filename = ftp_upload_log.replace(".log", "") + todaydate.strftime("-%Y-%m-%d") + ".log"      #File name written on server
    dirpath = ""
    filepath = os.path.join(dirpath, ftp_upload_log)
    save_log_file(filename,filepath, ftp_upload_log)

def save_log_file_yesterday():     #To save yesterday's file name
    todaydate = datetime.date.today()
    ...
    base = str.replace("1","")
    filename = ftp_upload_log.replace(".log", "") + yesterday.strftime("-%Y-%m-%d") + ".log"      #File name written on server
    save_log_file(filename,filepath, yesterday_log)

def save_log_file(filename, filepath, logfile_name):                #This is called when the scheduler decides it is time to save the log file
    ...
    if os.path.isfile(logfile_name):       #Check to see if the log file exists
...
def main():
    ...
    schedule.every(save_log_frequently).minutes.do(save_log_file_today)  # save the log_file
    schedule.every().day.at(save_log_time).do(save_log_file_yesterday)      #save the log file once a day
    try:
